...
export class ListaCircularSimpleComponent implements OnInit {
  ...
  async addData(numero) {
    if (!this.repetidos) {
      let temp = this.listaSimpleCircular.search(this.convertir(numero))
      if (temp !== null) {
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
    ...
    if (this.alfinal) {
      ...
      await this.listaSimpleCircular.add(this.convertir(numero), this.svg1, dibujo, `${this.velocidad}s`)
    }

    if (this.alinicio) {
      let dibujo = document.getElementById("cuerpoDraw")
      await this.listaSimpleCircular.addAlInicio(this.convertir(numero), this.svg1, dibujo, `${this.velocidad}s`)
    }

    if (this.ordenado) {
      let dibujo = document.getElementById("cuerpoDraw")
      await this.listaSimpleCircular.addOrdenado(this.convertir(numero), this.svg1, dibujo, `${this.velocidad}s`)
    }
    return 1
  }
  ...
  async search() {
    let result = await this.listaSimpleCircular.searchAnimation(this.convertir(this.numeroBuscar), `${this.velocidad}s`)
    if (result === null) {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
  ...
  async delete() {
    let result = await this.listaSimpleCircular.eliminar(this.convertir(this.numeroEliminar), `${this.velocidad}s`, this.svg1)
    if (result === -1) {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
  ...
  async actualizar() {
    let result = await this.listaSimpleCircular.searchAnimation(this.convertir(this.numeroAntiguo), `${this.velocidad}s`)
    if (result === null) {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
