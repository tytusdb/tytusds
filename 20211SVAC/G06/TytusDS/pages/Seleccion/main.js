...
async function SeleccionLabels() {
    ...
    for (let i = 0; i < MyChart.data.datasets[0].data.length; i++) {
        ...
        for (let j = i + 1; j < MyChart.data.datasets[0].data.length; j++) {
            let seg = 0
            let pri = 0
            for (k = 0; k < MyChart.data.labels[i].length; k++) {
                pri += MyChart.data.labels[i].toString().charCodeAt(k)
            }
            for (k = 0; k < MyChart.data.labels[j].length; k++) {
                seg += MyChart.data.labels[j].toString().charCodeAt(k)
            }
            if (pri > seg) {
