...
function rapido(listaV){
    ...
    bloques[iPiv].style.backgroundColor = "#FF4949";
    ...
    for(let n = 0; n < bloques.length; n++){
        container.removeChild(bloques[n]);
    }
    leftSide = rapido(leftSide);
    generateElements2(leftSide, tipoDato);

    rightSide = rapido(rightSide);
    generateElements2(rightSide, tipoDato);
    console.log("---------------------")
    return leftSide.concat(pivote).concat(rightSide);
    

}
function generateElements2(datos, typeD){
    for (var t = 0; t < datos.length;t++){
        // creando div para cada dato de la lista
        var elemento = document.createElement("div");
        elemento.classList.add("block");
