...
class Huffman {
	...
	toString() {
		let tmpString = this.cadena
		this.codificacion.forEach((value: ParejaHuffman) => {
			const char = value.valor.split(' - ')[0]
			// @ts-ignore
	...
	private buscar(binaria: string, raiz: NodoHuffman, cadena: string) {
		if (raiz != null) {
			...
			if (raiz.valor.length == 1) {
				this.codificacion.push(
					new ParejaHuffman(
						raiz.valor + ' - ' + raiz.peso + '/' + cadena.length,
						binaria,
						raiz.peso,
					),
				)
